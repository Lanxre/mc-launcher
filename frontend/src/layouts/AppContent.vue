...
const getUniqueLoaders = computed(() =>
  [...new Set(allMods.value.flatMap(m => m.Loaders || []).filter(Boolean))].sort()
)
...
const loadMoreMods = async () => {
  ...
  try {
    ...
    const newMods = uniqueBy(result.filter(m => !existingUrls.has(m.ModPageLink)), m => m.Name)
...
const applySearch = async () => {
  resetPaging()
  if (!searchE.value) {
    updateMods(modStore.getAllMods.length ? modStore.getAllMods : allMods.value)
...
const initObserver = async (el: HTMLElement) => {
  ...
  observer = new IntersectionObserver(([entry]) => {
    if (entry?.isIntersecting && hasMore.value && !loadingMore.value) loadMoreMods()
